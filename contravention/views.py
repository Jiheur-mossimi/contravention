...
def mcontact_views(requete, cont):
    mc = Contact.objects.get(id=cont)
    assujettis = Assujettis.objects.all()
...
def mpaiement_views(requete, paie):
    mp = Paiement.objects.get(id=paie)
    amendes =Amende.objects.all()
...
def massujettis_views(requete, ass):
    asm = Assujettis.objects.get(id=ass)
    ctx ={
        'ass': asm,
...
def scontact_view(requete, cont):
    c_s = Contact.objects.get(id=cont)
    c_s.delete()
    contacts = Contact.objects.all()
    ctx = {
        'contacts' : contacts,
...
def spaiement_view(requete, paie):
    ...
    return render(requete, 'appcontra/paiements.html', ctx) 
        ...
        # si c'est nouveau
# ------------------------------------------
        # A partir des codes ci-dessous, ils ne marchents pas totalement comme telle, il faut les revoirs.
...
def epaiement_views(requete):
    ...
    am = Amende.objects.get(id=am_code)

    rse = Paiement.objects.filter(id=i)
...
def einfraction_views(requete):
    ...
    ass = Assujettis.objects.get(id=ass_code)
...
def eamende_views(requete):
    ...
    rse = Amende.objects.filter(id=i)
...
def eassujettis_views(requete):
    ...
    d = requete.POST.get('date')
    a = requete.POST['adresse']
    t = requete.POST['telephone']
    rss = Assujettis.objects.filter(id=i)
...
def emassujettis_views(requete):
    ...
    a = requete.POST['adresse']
    t = requete.POST['telephone']
    rsp = Assujettis.objects.filter(id=i)
